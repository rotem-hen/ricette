...
export class RecipesComponent implements OnInit, OnDestroy {
  ...
  private applySearch(value: string): void {
    ...
    history.replaceState(
      history.state,
      '',
      `categories/${SpecialCategories.SEARCH_RESULTS}?${this.QUERY_PARAM}=${value}`
    );
    this.categoryName = `${this.searchCategoryName}: ${value}`;
    this.recipeList = this.allRecipes.filter(r =>
      `${r.title}\n${r.ingredients}`.toLowerCase().includes(value.toLowerCase())
    );
  }
